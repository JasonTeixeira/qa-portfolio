...
export const metadata: Metadata = {
  title: 'The Method — how we forge senior engineers — Sage Academy',
  description:
  ...
  alternates: { canonical: `${SITE}/academy/method` },
  openGraph: {
    title: 'The Method — how Sage forges senior engineers',
    description: 'Mental models, systems thinking, and a proof-driven loop — the system behind the difference between knowing syntax and being an engineer.',
      ...
      <div style={{ background: '#0B0B0E', color: INK, fontFamily: 'var(--font-sans), sans-serif', overflowX: 'clip' }}>
        ...
        <header style={{ ...container, backgroundImage: 'radial-gradient(90% 55% at 50% -8%, rgba(61,90,254,0.09) 0%, transparent 58%)' }}>
          ...
          <p style={{ margin: '26px 0 0', color: DIM, fontSize: 'clamp(17px, 1.5vw, 20px)', lineHeight: 1.6, maxWidth: '58ch', textWrap: 'pretty' }}>
            Anyone can look up syntax. What separates a senior engineer is judgment — the mental models, the systems
            thinking, and the nerve to defend a decision at 3am. That can be built. This is the system that builds it.
          </p>
          <div style={{ display: 'flex', flexWrap: 'wrap', gap: 12, marginTop: 34 }}>
            <Link href="/academy/signup" style={{ display: 'inline-flex', background: ACCENT, color: '#fff', textDecoration: 'none', fontSize: 15, fontWeight: 600, padding: '15px 28px', borderRadius: 26, boxShadow: '0 0 22px rgba(61,90,254,0.35)' }}>Start free</Link>
            <Link href="/academy/map" style={{ display: 'inline-flex', color: INK, border: `1px solid #2A2A33`, textDecoration: 'none', fontSize: 15, padding: '14px 28px', borderRadius: 26 }}>See the full path →</Link>
          </div>
        </header>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(min(100%, 280px), 1fr))', gap: 18, marginTop: 40 }}>
              {GAP.map((g) => (
                <div key={g.t} style={{ border: `1px solid ${LINE}`, borderRadius: 14, background: '#111115', padding: 24 }}>
                  <div style={{ ...serif, fontSize: 20, fontWeight: 600, color: INK, letterSpacing: '-0.01em' }}>{g.t}</div>
                  <p style={{ margin: '10px 0 0', fontSize: 14.5, color: DIM, lineHeight: 1.6 }}>{g.s}</p>
                </div>
              ))}
            </div>
          ...
          <div style={container}>
            <div style={{ maxWidth: 640 }}>
              ...
              <p style={{ margin: '18px 0 0', color: DIM, fontSize: 16.5, lineHeight: 1.6, maxWidth: '56ch' }}>
                Ten moves, one arc — repeated until it’s reflex. You don’t memorize it; you build the muscle by doing it
                every single lesson, on real problems, with a check at the end.
              </p>
            </div>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(min(100%, 320px), 1fr))', gap: 18, marginTop: 40 }}>
              {SCIENCE.map((f) => (
                <div key={f.tag} style={{ border: `1px solid ${LINE}`, borderRadius: 16, background: '#111115', padding: 26 }}>
                  <div style={{ ...mono, fontSize: 10.5, textTransform: 'uppercase', letterSpacing: '0.14em', color: f.color }}>{f.tag}</div>
                  <div style={{ ...serif, margin: '12px 0 0', fontSize: 21, fontWeight: 600, color: INK, letterSpacing: '-0.015em' }}>{f.t}</div>
                  <p style={{ margin: '12px 0 0', fontSize: 14.5, color: DIM, lineHeight: 1.65 }}>{f.s}</p>
                </div>
              ))}
            </div>
          </div>
          ...
          <div style={container}>
            <div style={{ maxWidth: 660 }}>
              ...
              <p style={{ margin: '18px 0 0', color: DIM, fontSize: 16.5, lineHeight: 1.6, maxWidth: '58ch' }}>
                Every concept you learn connects to the ones around it. Over time it becomes a knowledge graph you own —
                transferable structures that work on a codebase you’ve never seen, a system you’ve never designed, an
                outage you’ve never faced.
              </p>
            </div>
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(min(100%, 260px), 1fr))', gap: 18, marginTop: 40 }}>
              {MODELS.map((m) => (
                <div key={m.n} style={{ border: `1px solid ${LINE}`, borderRadius: 14, background: '#111115', padding: 24 }}>
                  ...
                  <div style={{ ...serif, margin: '10px 0 0', fontSize: 19, fontWeight: 600, color: INK, letterSpacing: '-0.01em' }}>{m.t}</div>
                  <p style={{ margin: '10px 0 0', fontSize: 14, color: DIM, lineHeight: 1.6 }}>{m.s}</p>
                </div>
              ))}
            </div>
          </div>
        ...
        <section style={section}>
          <div style={{ ...container, textAlign: 'center' }}>
            <div style={{ ...kicker, textAlign: 'center' }}>The outcome</div>
            <h2 style={{ ...serif, margin: '16px auto 0', fontWeight: 600, fontSize: 'clamp(30px, 4.4vw, 56px)', lineHeight: 1.04, letterSpacing: '-0.025em', maxWidth: '20ch', textWrap: 'balance' }}>
              An engineer who can frame, map, decide, and <em style={{ fontStyle: 'italic', color: GREEN }}>prove</em> it.
            </h2>
            <p style={{ margin: '22px auto 0', color: DIM, fontSize: 17, lineHeight: 1.6, maxWidth: '52ch' }}>
              Not someone who finished a course. Someone who walks into a design review, draws the system, defends the
              edges, and ships proof a skeptic can run. That’s the bar. That’s what we build.
            </p>
            <div style={{ display: 'flex', justifyContent: 'center', flexWrap: 'wrap', gap: 12, marginTop: 34 }}>
              <Link href="/academy/signup" style={{ display: 'inline-flex', background: ACCENT, color: '#fff', textDecoration: 'none', fontSize: 15.5, fontWeight: 600, padding: '16px 32px', borderRadius: 28, boxShadow: '0 0 28px rgba(61,90,254,0.4)' }}>Start with Engineering Judgment</Link>
              <Link href="/academy/concepts" style={{ ...mono, fontSize: 12, color: ACCENT_INK, textDecoration: 'none', alignSelf: 'center' }}>read a lesson free →</Link>
            </div>
          </div>
        </section>
      </div>
