    ...
    return (
      <div className={styles.page}>
        <div className={styles.gate}>
          ...
          <Link href="/login?next=/academy/dashboard" className={styles.gateBtn}>Sign in →</Link>
          <Link href="/academy/catalog" className={styles.gateAlt}>Browse the catalog</Link>
        </div>
      </div>
    )
  }
  ...
  return (
    <div className={styles.page}>
      ...
      <div className={styles.stats}>
        <div className={styles.stat}><dt>{dash.lessonsCompleted}</dt><dd>lessons completed</dd></div>
        <div className={styles.stat}><dt>{dash.coursesInProgress}</dt><dd>in progress</dd></div>
        <div className={styles.stat}><dt>{dash.coursesCompleted}</dt><dd>courses finished</dd></div>
        <div className={styles.stat}><dt>{dash.certificates.length}</dt><dd>certificates</dd></div>
      </div>

      {dash.continueTo ? (
        <Link href={`/academy/learn/${dash.continueTo.courseSlug}/${dash.continueTo.lessonSlug}`} className={styles.resume}>
          <span className={styles.resumeAccent} aria-hidden="true" />
          <span className={styles.resumeKicker}>▸ Pick up where you left off</span>
          <span className={styles.resumeTitle}>{dash.continueTo.lessonSlug.replace(/-/g, ' ').replace(/\b\w/g, (c) => c.toUpperCase())}</span>
      ...
      <section className={styles.section}>
        ...
        {dash.courses.length === 0 ? (
          <p className={styles.empty}>You haven’t started a course yet. <Link href="/academy/catalog">Browse the catalog →</Link></p>
        ) : (
          <div className={styles.courseGrid}>
            {dash.courses.map((c) => {
              ...
              return (
                <Link key={c.slug} href={`/academy/course/${c.slug}`} className={styles.courseCard} style={tvars(t)}>
                  <span className={styles.courseTag}>{TOPICS[c.topic].label}</span>
                  <h3 className={styles.courseTitle}>{c.title}</h3>
                  <span className={styles.bar} aria-hidden="true"><span style={{ width: `${c.pct}%` }} /></span>
                  <span className={styles.courseMeta}>{c.done} / {c.total} lessons · {c.pct}%</span>
        ...
        <section className={styles.section}>
          ...
          <div className={styles.certGrid}>
            {dash.certificates.map((cert) => {
              ...
              return (
                <Link key={cert.code} href={`/academy/certificate/${cert.code}`} className={styles.certCard} style={tvars(t)}>
          ...
          <ul className={styles.pathList}>
            {dash.paths.map((p) => (
              <li key={p.id} className={styles.pathRow}>
                ...
                <Link href="/academy/build" className={styles.pathEdit}>Edit →</Link>
              </li>
            ))}
          </ul>
        </section>
