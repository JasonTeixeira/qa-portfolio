...
@media (prefers-reduced-motion: reduce) { .sgEdge { animation: none !important; opacity: 1 !important; } }`,
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(min(100%, 340px), 1fr))', gap: 'clamp(36px, 5vw, 72px)', alignItems: 'center' }}>
            <div style={{ minWidth: 0 }}>
              <div style={{ ...mono, fontSize: 11.5, textTransform: 'uppercase', letterSpacing: '0.16em', color: '#8FA0FF' }}>
                Engineering judgment · proven, not claimed
              </div>
              <h1 style={{ ...serif, margin: '20px 0 0', fontWeight: 600, fontSize: 'clamp(42px, 5.6vw, 78px)', lineHeight: 0.99, letterSpacing: '-0.028em', textWrap: 'balance' }}>
                Learn to think like a senior engineer — and <em style={{ fontStyle: 'italic', fontWeight: 500, color: '#8FA0FF' }}>prove</em> it.
              </h1>
              <p style={{ margin: '22px 0 0', color: '#9C9CA6', fontSize: 'clamp(16px, 1.3vw, 18px)', maxWidth: '50ch', textWrap: 'pretty' }}>
                {coursesCount} courses that end in evidence a reviewer trusts — not a certificate of completion.
              </p>
              ...
              <div style={{ display: 'flex', gap: 28, marginTop: 36, borderTop: `1px solid ${LINE}`, paddingTop: 22, flexWrap: 'wrap' }}>
                ...
                <div><span style={{ ...serif, fontWeight: 600, fontSize: 24 }}>14</span> <span style={{ fontSize: 13, color: '#9598A2' }}>blocks per lesson</span></div>
                <div><span style={{ ...serif, fontWeight: 600, fontSize: 24, color: GREEN }}>100%</span> <span style={{ fontSize: 13, color: '#9598A2' }}>proofs verifiable</span></div>
              </div>
            </div>
        ...
        <div style={{ borderTop: `1px solid ${LINE}`, background: '#0D0D11' }}>
          <div style={{ maxWidth: 1240, margin: '0 auto', padding: '13px clamp(20px, 4vw, 48px)', display: 'flex', alignItems: 'center', gap: 18, overflow: 'hidden' }}>
            <span style={{ ...mono, display: 'inline-flex', alignItems: 'center', gap: 8, fontSize: 10, textTransform: 'uppercase', letterSpacing: '0.12em', color: GREEN, flexShrink: 0 }}>
              <span style={{ width: 7, height: 7, borderRadius: '50%', background: GREEN }} />live
            </span>
            <HeroTicker />
            <span style={{ ...mono, marginLeft: 'auto', fontSize: 11, color: '#9598A2', whiteSpace: 'nowrap', flexShrink: 0 }}>
              <span style={{ color: INK }}>{lessonsCount}</span> lessons live
            </span>
          </div>
        </div>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(min(100%, 320px), 1fr))', gap: 'clamp(32px, 5vw, 72px)', alignItems: 'center' }}>
              <div>
                ...
                <p style={{ margin: '18px 0 0', color: '#9C9CA6', maxWidth: '46ch', textWrap: 'pretty' }}>
                  We do — and we make you prove it. The difference between a pile of tutorials and a senior engineer is a map you can defend the edges of.
                </p>
              </div>
              <figure style={{ margin: 0, position: 'relative', display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 1, background: '#2A2A33', border: `1px solid ${LINE}`, borderRadius: 14, overflow: 'hidden' }}>
                <div style={{ background: '#111115', padding: 22 }}>
                  <div style={{ ...mono, fontSize: 10, textTransform: 'uppercase', letterSpacing: '0.12em', color: AMBER, marginBottom: 14 }}>▲ Weak · a bag of boxes</div>
                  <div style={{ display: 'flex', flexWrap: 'wrap', gap: 8, opacity: 0.7, marginBottom: 16 }}>
                    {['API', 'DB', 'Queue?', '…stuff'].map((s) => (
                      <span key={s} style={{ border: '1px solid #2A2A33', borderRadius: 6, padding: '6px 12px', fontSize: 12, color: '#9598A2' }}>{s}</span>
                    ))}
                  </div>
                  <div style={{ fontSize: 13, color: '#9C9CA6' }}>Confident guesses in a war room. Edges nobody can defend.</div>
                </div>
                <div style={{ background: '#111115', padding: 22 }}>
                  <div style={{ ...mono, fontSize: 10, textTransform: 'uppercase', letterSpacing: '0.12em', color: GREEN, marginBottom: 14 }}>✓ Gold · a decision map</div>
                  <div style={{ display: 'flex', alignItems: 'center', gap: 6, flexWrap: 'wrap', marginBottom: 16 }}>
                    <span style={{ border: '1.5px solid #8FA0FF', borderRadius: 6, padding: '6px 10px', fontSize: 12, background: 'rgba(61,90,254,0.08)' }}>checkout</span>
                    <span style={{ color: '#4A4A54' }}>→</span>
                    <span style={{ border: `1.5px solid ${AMBER}`, borderRadius: 6, padding: '6px 10px', fontSize: 12, background: 'rgba(224,169,62,0.08)' }}>retry path</span>
                    <span style={{ color: '#4A4A54' }}>→</span>
                    <span style={{ border: `1.5px solid ${GREEN}`, borderRadius: 10, padding: '6px 10px', fontSize: 12, background: 'rgba(24,182,99,0.08)' }}>ledger</span>
                  </div>
                  <div style={{ fontSize: 13, color: '#B6B6C0' }}>The failure is a location. The omissions are defended.</div>
                </div>
                <div style={{ ...serif, position: 'absolute', left: '50%', top: '50%', transform: 'translate(-50%, -50%)', width: 36, height: 36, borderRadius: '50%', background: '#0B0B0E', border: '1px solid #2A2A33', display: 'grid', placeItems: 'center', fontStyle: 'italic', fontSize: 13, color: '#9598A2', zIndex: 2 }}>vs</div>
              </figure>
            </div>
          </div>
        </section>
          ...
          <div style={container}>
            <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: 32, flexWrap: 'wrap' }}>
              <div style={{ maxWidth: 660, flex: '1 1 380px' }}>
                ...
                <h2 style={h2}>Every course is an instance of the loop senior engineers run on autopilot.</h2>
              </div>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(190px, 1fr))', gap: 1, marginTop: 44, background: LINE, border: `1px solid ${LINE}`, borderRadius: 14, overflow: 'hidden' }}>
              {LOOP.map(([name, desc], i) => (
                <div key={name} style={{ background: '#111115', padding: 20, minHeight: 118, display: 'flex', flexDirection: 'column', gap: 8 }}>
                  ...
                  <div style={{ fontSize: 12.5, color: '#9598A2', lineHeight: 1.45 }}>{desc}</div>
                </div>
              ))}
            </div>
          </div>
          ...
          <div style={container}>
            <div style={{ maxWidth: 660, marginBottom: 44 }}>
              ...
              <p style={{ margin: 0, color: '#9C9CA6', maxWidth: '60ch', textWrap: 'pretty' }}>
                Every lesson is a deliberate 14-block arc: a contract, a stake, one mental model, three hero visuals, a lab that starts failing, and a verification you can&apos;t hand-wave.
              </p>
            </div>
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(min(100%, 320px), 1fr))', gap: 20 }}>
              <div style={{ border: `1px solid ${LINE}`, borderRadius: 14, background: '#111115', padding: 26 }}>
                <div style={{ ...mono, fontSize: 10, textTransform: 'uppercase', letterSpacing: '0.12em', color: '#9598A2', marginBottom: 18 }}>The 14-block arc</div>
                <div style={{ display: 'flex', flexDirection: 'column' }}>
                  {ARC.map((a) => (
                    <div key={a.num} style={{ display: 'grid', gridTemplateColumns: '18px 110px 1fr', gap: 12, alignItems: 'baseline', padding: '7px 0' }}>
                      ...
                      <span style={{ fontSize: 14, fontWeight: a.weight, color: a.ink }}>{a.name}</span>
                      <span style={{ fontSize: 12.5, color: '#9598A2' }}>{a.desc}</span>
                    </div>
                  ))}
                </div>
              </div>
              <div style={{ border: `1px solid ${LINE}`, borderRadius: 14, background: '#111115', overflow: 'hidden', display: 'flex', flexDirection: 'column' }}>
                <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '13px 18px', borderBottom: `1px solid ${LINE}` }}>
                  <span style={{ ...mono, fontSize: 10, textTransform: 'uppercase', letterSpacing: '0.12em', color: '#9598A2' }}>In-browser lab · Python</span>
                  <span style={{ ...mono, fontSize: 10, color: AMBER }}>starter fails — on purpose</span>
                </div>
                ...
                <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '13px 18px', borderTop: `1px solid ${LINE}` }}>
                  <span style={{ ...mono, fontSize: 11, color: '#E5484D' }}>✗ check failed · 1 expected, 3 found</span>
                  <Link href="/academy/engine" style={{ display: 'inline-flex', background: ACCENT, color: '#fff', fontSize: 12.5, fontWeight: 600, padding: '8px 16px', borderRadius: 18, textDecoration: 'none', whiteSpace: 'nowrap' }}>Run again</Link>
                </div>
          ...
          <div style={container}>
            <div style={{ display: 'flex', flexWrap: 'wrap', alignItems: 'flex-end', justifyContent: 'space-between', gap: 20, marginBottom: 40 }}>
              <div style={{ maxWidth: 620 }}>
                ...
                <h2 style={h2}>Six courses live. Seventeen more on the same engine.</h2>
              </div>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(330px, 1fr))', gap: 18 }}>
              {COURSES.map((c) => (
                <Link key={c.name} href={c.href} style={{ border: `1px solid ${LINE}`, borderRadius: 14, background: '#111115', padding: 24, display: 'flex', flexDirection: 'column', gap: 14, cursor: 'pointer', textDecoration: 'none', color: 'inherit' }}>
                  <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
                    <span style={{ ...mono, fontSize: 9.5, textTransform: 'uppercase', letterSpacing: '0.1em', color: c.tint }}>{c.track}</span>
                    <span style={{ ...mono, fontSize: 9.5, color: GREEN }}>✓ live</span>
                  </div>
                  <div style={{ display: 'flex', alignItems: 'center', gap: 14 }}>
                    ...
                    <span style={{ ...serif, fontWeight: 600, fontSize: 21, letterSpacing: '-0.015em', lineHeight: 1.15 }}>{c.name}</span>
                  </div>
                  ...
                  <div style={{ ...mono, display: 'flex', justifyContent: 'space-between', borderTop: `1px solid ${LINE}`, paddingTop: 13, fontSize: 10.5, color: '#9598A2' }}>
                    <span>{c.meta}</span>
                    <span style={{ color: c.tint }}>artifact: {c.artifact}</span>
                  </div>
                </Link>
              ))}
            </div>
            <div style={{ marginTop: 18, border: '1px dashed #2A2A33', borderRadius: 14, padding: '20px 24px', display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: 16, flexWrap: 'wrap' }}>
              <span style={{ fontSize: 14.5, color: '#9C9CA6' }}>
                <span style={{ color: INK, fontWeight: 600 }}>+17 in production</span> — AI Engineering &amp; RAG, Security, Cloud &amp; Ops, Observability, System Design, Leadership…
              </span>
              <Link href="/academy/signup" style={{ ...mono, fontSize: 11.5, color: '#8FA0FF', textDecoration: 'none' }}>Notify me →</Link>
            </div>
          </div>
        </section>
          ...
          <div style={container}>
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(min(100%, 320px), 1fr))', gap: 'clamp(32px, 5vw, 72px)', alignItems: 'center' }}>
              <div>
                ...
                <h2 style={{ ...h2, margin: '14px 0 16px' }}>You leave with a portfolio, not a receipt.</h2>
                <p style={{ margin: 0, color: '#9C9CA6', maxWidth: '46ch', textWrap: 'pretty' }}>
                  Decision memos and passing proofs — pick any claim, follow the artifact, see for yourself. Certificates are auto-issued on genuine completion, verifiable by code.
                </p>
                {/* eslint-disable-next-line @next/next/no-img-element */}
                <img
              ...
              <div style={{ border: `1px solid ${LINE}`, borderRadius: 14, background: '#111115', overflow: 'hidden' }}>
                <div style={{ ...mono, display: 'grid', gridTemplateColumns: 'minmax(120px, 1.2fr) minmax(120px, 1.2fr) auto', gap: 14, padding: '12px 22px', borderBottom: `1px solid ${LINE}`, fontSize: 9.5, textTransform: 'uppercase', letterSpacing: '0.1em', color: '#4A4A54' }}>
                  <span>Claim</span><span>Artifact</span><span>Verdict</span>
                </div>
                {EVIDENCE.map((e) => (
                  <div key={e.claim} style={{ display: 'grid', gridTemplateColumns: 'minmax(120px, 1.2fr) minmax(120px, 1.2fr) auto', gap: 14, alignItems: 'center', padding: '15px 22px', borderBottom: `1px solid ${LINE}` }}>
                    <span style={{ fontSize: 13.5, fontWeight: 500 }}>{e.claim}</span>
                    <span style={{ ...mono, fontSize: 11, color: '#8FA0FF' }}>{e.artifact}</span>
                    <span style={{ ...mono, fontSize: 9.5, letterSpacing: '0.08em', padding: '4px 9px', borderRadius: 5, color: e.c, border: `1px solid ${e.b}`, background: e.bg, whiteSpace: 'nowrap' }}>{e.verdict}</span>
                  </div>
                ))}
                <div style={{ ...mono, padding: '13px 22px', fontSize: 10.5, color: '#9598A2', display: 'flex', justifyContent: 'space-between', gap: 12, flexWrap: 'wrap' }}>
                  <span>sample ledger · certificates verifiable at /verify</span>
                  <Link href="/academy/evidence" style={{ color: '#8FA0FF', textDecoration: 'none' }}>share portfolio →</Link>
                </div>
              </div>
            </div>
          </div>
          ...
          <div style={container}>
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(min(100%, 400px), 1fr))', gap: 20 }}>
              <div style={{ border: `1px solid ${LINE}`, borderRadius: 14, background: '#111115', padding: 28, display: 'flex', flexDirection: 'column', gap: 16 }}>
                ...
                <h3 style={{ ...serif, margin: 0, fontWeight: 560, fontSize: 24, letterSpacing: '-0.015em' }}>Your score is capped by your weakest proof.</h3>
                <p style={{ margin: 0, fontSize: 14, color: '#9C9CA6', textWrap: 'pretty' }}>
                  Because that&apos;s how a reviewer reads it. We show you the one repair that lifts it.
                </p>
                <div style={{ marginTop: 8 }}>
                  <div style={{ ...mono, display: 'flex', justifyContent: 'space-between', fontSize: 10.5, color: '#9598A2', marginBottom: 8 }}>
                    <span>MASTERY</span>
                    <span><span style={{ color: AMBER }}>78</span> · capped by PROOF</span>
                  </div>
                  ...
                  <div style={{ display: 'flex', justifyContent: 'space-between', marginTop: 10, fontSize: 12.5 }}>
                    <span style={{ color: '#9598A2' }}>ceiling until the proof passes</span>
                    <span style={{ color: GREEN, fontWeight: 600 }}>repair lesson 10 → lifts to 91</span>
                  </div>
                </div>
              </div>
              <div style={{ border: `1px solid ${LINE}`, borderRadius: 14, background: '#111115', padding: 28, display: 'flex', flexDirection: 'column', gap: 16 }}>
                <div style={{ ...mono, fontSize: 10, textTransform: 'uppercase', letterSpacing: '0.12em', color: '#9598A2' }}>Spaced recall · 1 / 3 / 7 / 30 days</div>
                <h3 style={{ ...serif, margin: 0, fontWeight: 560, fontSize: 24, letterSpacing: '-0.015em' }}>It holds under pressure — not just until the quiz.</h3>
                ...
                <div style={{ ...mono, display: 'flex', gap: 20, fontSize: 10.5, color: '#9C9CA6' }}>
                  <span><span style={{ color: '#8FA0FF' }}>—</span> with recall</span>
                  <span><span style={{ color: '#4A4A54' }}>--</span> cram once</span>
                  <span style={{ marginLeft: 'auto', color: GREEN }}>the whole point</span>
                </div>
              </div>
            </div>
          </div>
        ...
        <section style={{ ...section, background: '#0D0D11', overflow: 'hidden' }}>
          <div style={{ maxWidth: 1240, margin: '0 auto', padding: 'clamp(72px, 10vw, 140px) clamp(20px, 4vw, 48px)', position: 'relative' }}>
            ...
            />
            <div style={{ ...mono, fontSize: 11, textTransform: 'uppercase', letterSpacing: '0.18em', color: '#9598A2' }}>The house rule</div>
            <div style={{ ...serif, marginTop: 20, fontStyle: 'italic', fontWeight: 500, fontSize: 'clamp(40px, 6.4vw, 92px)', lineHeight: 1.02, letterSpacing: '-0.03em', maxWidth: '14ch', textWrap: 'balance' }}>
              Prove it — <span style={{ color: GREEN }}>no vibes.</span>
            </div>
        ...
        <section id="offer" style={{ ...section, background: '#0D0D11' }}>
          <div style={{ maxWidth: 900, margin: '0 auto', padding: 'clamp(64px, 9vw, 120px) clamp(20px, 4vw, 48px)', textAlign: 'center' }}>
            <div style={kicker}>All-access membership</div>
            ...
            <p style={{ margin: '20px auto 0', color: '#9C9CA6', maxWidth: '52ch', fontSize: 16.5, textWrap: 'pretty' }}>
              All {coursesCount} courses as they ship, every lab and proof, spaced recall, leagues, and verifiable certificates. Value framed as outcome — judgment plus evidence — not hours of video.
            </p>
