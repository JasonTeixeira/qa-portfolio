...
export const hbars = (data, o = {}) => { const w = 300, h = data.length * 34 + 12, max = Math.max(...data.map(d => d.v));
...
export const groupedBars = (groups, o = {}) => { const w = 300, h = 180, p = 22, gv = groups.flatMap(g => g.v), max = Math.max(...gv), gw = (w - p * 2) / groups.length;
...
export const candles = (data, o = {}) => { const w = 300, h = 180, p = 16, all = data.flatMap(d => [d.h, d.l]), max = Math.max(...all), min = Math.min(...all), bw = (w - p * 2) / data.length;
...
export const scatter = (pts, o = {}) => { const w = 300, h = 190;
...
export const gauge = (pct, o = {}) => { const a = lerp(20, 160, pct / 100), rad = (160 - a) * Math.PI / 180, ex = 90 + 70 * Math.cos(Math.PI - a * Math.PI / 180), ey = 110 - 70 * Math.sin(a * Math.PI / 180);
...
export const pyramid = (levels, o = {}) => { const w = 240, h = 170;
...
export const cycle = (nodes, o = {}) => { const R = 78, cx = 110, cy = 110; return `<div style="position:relative;width:220px;height:220px">${nodes.map((n, i) => { const a = i * (2 * Math.PI / nodes.length) - Math.PI / 2, x = cx + R * Math.cos(a), y = cy + R * Math.sin(a), c = n.c || hue(i);
...
export const layered = (layers, o = {}) => `<div style="display:flex;flex-direction:column;gap:8px;width:260px">${layers.map((l, i) => `<div style="border:1.5px solid var(--${l.c || hue(i)});border-radius:10px;padding:12px;text-align:center;font-family:var(--mono);font-size:13px;color:var(--${l.c || hue(i)});display:flex;align-items:center;justify-content:center;gap:8px">${l.icon ? icon(l.icon, 18) : ''}${l.l}</div>`).join('')}</div>`;

export const tree = (root, o = {}) => { // {l, children:[...]} one level shown
...
export const network = (nodes, links, o = {}) => svg(220, 170, links.map(l => `<line x1="${nodes[l[0]].x}" y1="${nodes[l[0]].y}" x2="${nodes[l[1]].x}" y2="${nodes[l[1]].y}" stroke="var(--line2)"/>`).join('') + nodes.map((n, i) => `<circle cx="${n.x}" cy="${n.y}" r="${n.r || 9}" fill="var(--${n.c || hue(i)})"/>`).join(''));

export const sequence = (actors, msgs, o = {}) => { const w = 260, lane = w / (actors.length + 1);
...
export const gantt = (tasks, o = {}) => svg(240, tasks.length * 26 + 20, `<line x1="0" y1="12" x2="240" y2="12" stroke="var(--line2)"/>` + tasks.map((t, i) => `<rect x="${t.s * 2.2}" y="${20 + i * 26}" width="${t.d * 2.2}" height="15" rx="4" fill="var(--${t.c || hue(i)})" opacity=".85"/><text x="${t.s * 2.2 + 4}" y="${31 + i * 26}" font-size="9" fill="#0B0B0E">${t.l || ''}</text>`).join(''));

export const venn = (a, b, o = {}) => svg(200, 150, `<circle cx="78" cy="75" r="50" fill="var(--brand)" fill-opacity=".22" stroke="var(--brand2)"/><circle cx="122" cy="75" r="50" fill="var(--db)" fill-opacity=".22" stroke="var(--db)"/><text x="55" y="79" text-anchor="middle" font-size="11" fill="var(--ink)">${a}</text><text x="145" y="79" text-anchor="middle" font-size="11" fill="var(--ink)">${b}</text>`);
...
export const diff = (lines, o = {}) => `<div style="font-family:var(--mono);font-size:15px;text-align:left;border-radius:10px;overflow:hidden;min-width:300px">${lines.map(l => `<div style="padding:4px 12px;color:var(--${l.s === '+' ? 'ok' : l.s === '-' ? 'failure' : 'muted'});background:${l.s === '+' ? 'rgba(94,224,140,.08)' : l.s === '-' ? 'rgba(255,107,107,.08)' : 'transparent'}">${l.s || ' '} ${esc(l.t)}</div>`).join('')}</div>`;
...
export const chat = (msgs, o = {}) => `<div style="display:flex;flex-direction:column;gap:12px;min-width:340px">${msgs.map(m => `<div style="align-self:${m.u ? 'flex-end' : 'flex-start'};max-width:80%;font-size:20px;padding:14px 18px;border-radius:16px;background:${m.u ? 'rgba(61,90,254,.14)' : m.bad ? 'rgba(255,107,107,.06)' : m.good ? 'rgba(94,224,140,.06)' : 'var(--s2)'};border:1px solid ${m.bad ? 'rgba(255,107,107,.4)' : m.good ? 'rgba(94,224,140,.4)' : 'var(--line2)'}">${m.t}</div>`).join('')}</div>`;

export const table = (head, rows, o = {}) => `<table style="border-collapse:collapse;font-family:var(--mono);font-size:15px"><tr>${head.map(h => `<td style="padding:8px 16px;color:var(--faint);border-bottom:1px solid var(--line2)">${h}</td>`).join('')}</tr>${rows.map(r => `<tr>${r.map((c, i) => `<td style="padding:8px 16px;border-bottom:1px solid var(--line);color:${c === '✓' ? 'var(--ok)' : c === '✗' ? 'var(--failure)' : i === 0 ? 'var(--muted)' : 'var(--ink)'}">${c}</td>`).join('')}</tr>`).join('')}</table>`;

export const steps = (items, o = {}) => `<div style="display:flex;flex-direction:column;gap:16px">${items.map((s, i) => `<div style="display:flex;align-items:center;gap:16px"><span style="width:36px;height:36px;border-radius:50%;border:2px solid var(--${s.c || hue(i)});color:var(--${s.c || hue(i)});display:grid;place-items:center;font-family:var(--mono);font-size:16px">${i + 1}</span><span style="font-family:var(--display);font-size:26px">${s.l}</span></div>`).join('')}</div>`;

export const chips = (items, o = {}) => `<div style="display:flex;flex-wrap:wrap;gap:10px;max-width:420px;justify-content:center">${items.map((t, i) => `<span style="font-family:var(--mono);font-size:15px;padding:8px 14px;border-radius:9px;border:1px solid var(--${hue(i)});color:var(--${hue(i)})">${t}</span>`).join('')}</div>`;

export const callout = (label, note, o = {}) => svg(240, 150, `<rect x="20" y="40" width="130" height="66" rx="10" fill="var(--s2)" stroke="var(--line2)"/><text x="36" y="78" font-size="14" fill="var(--ink)" font-family="var(--mono)">${label}</text><path d="M150,58 L205,32" stroke="var(--warn)" stroke-width="2"/><circle cx="205" cy="32" r="5" fill="var(--warn)"/><text x="176" y="22" font-size="11" fill="var(--warn)" font-family="var(--mono)">${note}</text>`);
