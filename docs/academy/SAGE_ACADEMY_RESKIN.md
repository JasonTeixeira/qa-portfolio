...
| Why Proof | Sage Why Proof | **DONE** — new /academy/why-proof editorial manifesto ("Prove it — no vibes.") modeled on About: manifesto prose + 4 decorative conceptual figures (war-room terminal, decision-map comparison, example mastery meter, claim→artifact→verdict list) + CTA row. Real CTAs verified: "Start with Engineering Judgment" → /academy/course/career-engineering_judgment_foundation, "See the loop" → /academy/resources/sprint-loop. Mockups in <figure> w/ aria-labels + the numeric one labeled "· example" (no live-stat fabrication). tsc clean; gates like sibling marketing (Phase 3 public). NOTE: subagent found /academy/how-it-works is NOT a real route (only a .dc.html) — verify the How-It-Works ledger entry's actual path in Phase 2/3. |
| Team Admin | Sage Team Admin | **DEFERRED → feature-build (needs team-plan backend)** — the design is a full academy team-plan admin product: team plan ($190/seat/yr), seat usage, member roster with roles (owner/admin/member), per-member sprint/proofs/mastery/ledger rollups, invite flow (send invite / copy team link / invite by email), cohort sprint progress, and team invoice billing (add seats, transfer ownership). NONE of this has a backend — academy_teams / academy_team_members / academy_seats are all REAL-MISSING (head-count false-positives; confirmed via real select), and the only "team" API (/api/admin/team/invite) is the AGENCY portal-admin inviting staff to `profiles`, not academy learner seats. Same no-team-backend finding as Cancel/Help/Legal. Building it means fabricating an entire team product + billing — build the backend first (teams/seats/roles/rollups + Stripe seat billing), then skin. |
