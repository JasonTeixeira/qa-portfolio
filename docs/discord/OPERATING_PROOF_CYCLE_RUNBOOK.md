...
6. After explicit approval, run `SAGE_ALLOW_NON_DRY_RAG_EVAL=approved npm run rag:evaluate:approved-missing` for routine missing-coverage repair. Run the full `rag:evaluate` drift check only when source quality or retrieval behavior changed materially.
