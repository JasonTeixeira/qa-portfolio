...
const courseSlug = 'career-observability_reliability_performance'
...
if (Object.keys(lessons).length !== 20 || JSON.stringify(Object.keys(lessons)) !== JSON.stringify(Object.keys(solutions))) throw new Error('Observability lesson/reference coverage drift')

const targets = {
  'observability-mindset': 'analyze',
  'service-level-objectives': 'slo_report',
  'golden-signals-red': 'red_metrics',
  'use-method-resources': 'use_sweep',
  'instrumentation-contracts': 'check_emissions',
  'structured-logging': 'query_logs',
  'distributed-tracing': 'analyze_trace',
  'trace-context-propagation': 'run_pipeline',
  'dashboards-as-code': 'diff_dashboards',
  'alert-design': 'evaluate',
  'burn-rate-alerting': 'classify',
  'incident-triage': 'triage',
  'incident-communication': 'comms',
  'postmortems': 'postmortem',
  'performance-profiling': 'report',
  'capacity-planning': 'knee',
  'queue-backpressure': 'simulate',
  'resilience-patterns': 'guarded',
  'release-observability': 'judge_release',
  'observability-capstone': 'build_packet',
}
...
function maskFunction(code, name, slug) {
  ...
  lines.splice(start + 1, end - start - 1,
    '    # TODO: implement the evidence decision from the supplied telemetry fixtures.',
    '    # Preserve failure discrimination and the exact observable contract.',
    '    raise NotImplementedError("complete the observability decision")',
    '',
  )
  return lines.join('\n')
}
...
for (const [lessonSlug, blocks] of Object.entries(lessons)) {
  ...
  const lab = {
    ...
    summary: `Use the supplied telemetry and incident fixtures to implement, execute, and explain the decision at the center of this lesson. ${contract.outcome} The exact output must distinguish healthy evidence from at least one burn, saturation, propagation, overload, or recovery failure.`,
    language: 'python', starter, check: exactOutput(solution, lessonSlug),
  }
  const worked = {
    type: 'worked-example',
    intro: `${walkthrough.title}. Trace the complete operator artifact before implementing its deterministic evidence model.`,
  ...
  const debug = {
    ...
    task: `Add a fixture that reproduces ${comparison.left.label}, make its user or operator impact observable, repair the evidence boundary, and retain the case as a regression check.`,
  ...
  before(blocks, 'transfer', {
    ...
    weak: `A dashboard or script runs, but the ${comparison.left.label} failure, user impact, exact evidence, or retained regression is missing.`,
...
console.log(`Upgraded ${Object.keys(lessons).length} Observability lessons with exact practice labs and calibrated reliability evidence.`)
