...
import { buildCareerContentHarness } from '@/lib/discord/career-content-harness';

const root = process.cwd();
const evidenceDir = path.join(root, 'docs', 'evidence', 'engineering-loop');
const outputPath = path.join(evidenceDir, 'career-content-harness-latest.json');
...
async function main() {
  const result = await buildCareerContentHarness({
    ...
    candidateLimit: Number(arg('candidate-limit') ?? 50),
    maxFiles: Number(arg('max-files') ?? 12000),
  });
...
main().catch(async (error) => {
  const result = {
    ok: false,
    version: 'career-content-harness-v1',
