  ...
  const evidence = {
    ...
    commandPlan: {
      ...
      mutationWarning: selectedKeys.length
        ? 'Approved command runs the guarded missing-eval repair sequence; it can call DeepSeek, run retrieval, upsert rag_eval_questions, insert rag_eval_runs/results, and update local eval evidence.'
    ...
    preRunChecks: [
      ...
      'Use npm run rag:evaluate:approved-missing for approved coverage repair so coverage readiness, final scorecard, and local evidence refresh in one guarded sequence.',
