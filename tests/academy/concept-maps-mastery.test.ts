...
const slug = 'career-concept_maps_real_world_engineering'
...
test('Concept Maps implements an ordered build-debug-defend mastery loop in all 30 lessons', () => {
...
test('every Concept Maps Python reference satisfies its exact observable contract', () => {
  let count = 0
  for (const [key, blocks] of Object.entries(lessons)) {
    const solution = solutions[key]
    const result = spawnSync('python3', ['-I', '-c', solution.code], { encoding: 'utf8', input: solution.stdin ?? '', timeout: 10000 })
...
test('Concept Maps gives review drills deep pacing and every defense capstone multi-day pacing', () => {
  for (const key of ['boundary-review-board', 'retrieval-without-the-map', 'socratic-map-review']) {
  ...
  for (const blocks of Object.values(lessons)) {
    const contract = blocks.find((block) => block.type === 'sprint-contract')
    if (contract?.intensity !== 'capstone') continue
  ...
  assert.equal(capstones, 8)
})

test('Concept Maps retains honest narration metadata', () => {
  let say = 0
  let audio = 0
  visit(lessons, (item) => {
...
test('Concept Maps has a unique primary and official source ledger', () => {
...
test('Concept Maps lesson sequence remains canonical registry truth', () => {
...
test('Academy audit includes Concept Maps mastery contract', () => {
  assert.match(JSON.parse(readFileSync('package.json', 'utf8')).scripts['academy:audit:test'], /concept-maps-mastery\.test\.ts/)
})
