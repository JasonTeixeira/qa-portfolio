...
const slug = 'career-frontend_fullstack'
...
test('Frontend and Full-Stack implements an ordered accessible production mastery loop in all 20 lessons', () => {
...
test('every Frontend and Full-Stack JavaScript reference satisfies its exact observable contract', () => {
  let count = 0
  for (const [key, blocks] of Object.entries(lessons)) {
...
test('Frontend and Full-Stack gives visual, integration, and deployment proof deep pacing and a multi-day capstone', () => {
  for (const key of ['responsive-visual-proof', 'fullstack-integration-workflow', 'deployment-env-observability']) {
  ...
  const blocks = lessons['fullstack-capstone']
  const contract = blocks.find((block) => block.type === 'sprint-contract')
...
test('Frontend and Full-Stack retains honest narration metadata', () => {
  let say = 0
  let audio = 0
  visit(lessons, (item) => {
...
test('Frontend and Full-Stack has a unique primary and official source ledger', () => {
...
test('Frontend and Full-Stack lesson sequence remains canonical registry truth', () => {
...
test('Academy audit includes Frontend and Full-Stack mastery contract', () => {
  assert.match(JSON.parse(readFileSync('package.json', 'utf8')).scripts['academy:audit:test'], /frontend-fullstack-mastery\.test\.ts/)
})
