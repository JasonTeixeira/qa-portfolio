...
test('Mobile Engineering implements an ordered device-to-production mastery loop in all 20 lessons', () => {
...
test('every Mobile Engineering Python reference satisfies its exact observable contract', () => {
  for (const [key, blocks] of Object.entries(lessons)) {
...
test('Mobile Engineering gives offline, security, accessibility, test, and release work deep pacing with a multi-day incident capstone', () => {
  for (const key of ['flaky-network-repair', 'secure-token-storage', 'privacy-safe-telemetry', 'mobile-accessibility', 'mobile-test-plan', 'crash-and-analytics-signals', 'app-store-readiness']) {
  ...
  const blocks = lessons['post-release-incident-loop']
  const contract = blocks.find((block) => block.type === 'sprint-contract')
...
test('Mobile Engineering retains honest narration metadata', () => {
  let say = 0
  let audio = 0
  visit(lessons, (item) => {
...
test('Mobile Engineering has a unique official and primary source ledger', () => {
...
test('Mobile Engineering lesson sequence remains canonical registry truth', () => {
...
test('Academy audit includes the Mobile Engineering mastery contract', () => {
  assert.match(JSON.parse(readFileSync('package.json', 'utf8')).scripts['academy:audit:test'], /mobile-engineering-mastery\.test\.ts/)
})
