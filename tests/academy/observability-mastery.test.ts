...
test('Observability implements one ordered, novice-scaffolded mastery loop in all 20 lessons', () => {
  ...
  for (const [lessonSlug, blocks] of Object.entries(lessons)) {
    ...
    assert.equal(blocks.filter((block) => block.type === 'mission').length, 1, `${key}: duplicate or missing mission`)
...
test('every Observability reference satisfies its exact observable contract', () => {
  const runtimeDir = mkdtempSync(join(tmpdir(), 'academy-observability-labs-'))
...
test('Observability pacing culminates in deep operational work and a multi-day capstone', () => {
  ...
  const capstone = lessons['observability-capstone']
...
test('Observability has a unique primary and official source ledger', () => {
  assert(existsSync(sourcePath), 'missing Observability source ledger')
...
test('the reliability-platform mapping enumerates every Observability lesson', () => {
...
test('the Academy audit gate includes the Observability mastery contract', () => {
  const packageJson = JSON.parse(readFileSync('package.json', 'utf8'))
  assert.match(packageJson.scripts['academy:audit:test'], /observability-mastery\.test\.ts/)
})
