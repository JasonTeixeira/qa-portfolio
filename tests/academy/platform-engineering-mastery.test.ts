...
test('Platform Engineering implements one ordered, novice-scaffolded mastery loop in all 20 lessons', () => {
...
test('every Platform Engineering reference satisfies its exact observable contract', () => {
  const runtimeDir = mkdtempSync(join(tmpdir(), 'academy-platform-labs-'))
...
test('Platform Engineering pacing culminates in deliberate practice and a multi-day platform capstone', () => {
  ...
  const capstone = lessons['platform-roadmap-review']
...
test('Platform Engineering has a unique primary and official source ledger', () => {
  assert(existsSync(sourcePath), 'missing Platform Engineering source ledger')
...
test('the reliability-platform mapping enumerates every Platform Engineering lesson', () => {
...
test('the Academy audit gate includes the Platform Engineering mastery contract', () => {
  const packageJson = JSON.parse(readFileSync('package.json', 'utf8'))
  assert.match(packageJson.scripts['academy:audit:test'], /platform-engineering-mastery\.test\.ts/)
})
