...
const container = `sageideas-billing-sql-${process.pid}-${randomUUID().slice(0, 8)}`
...
try {
  ...
  for (let attempt = 0; attempt < 60; attempt += 1) {
    ...
    const result = spawnSync('docker', [
      'exec', container, 'pg_isready', '-U', 'postgres', '-d', 'billing_test',
  ...
  const sql = await Promise.all([
    'tests/billing/sql/prelude.sql',
    'supabase/migrations/0121_billing_integrity.sql',
    'tests/billing/sql/assertions.sql',
  ].map((file) => readFile(path.join(root, file), 'utf8')))

  const output = run('docker', [
    'exec', '--interactive', container,
    'psql', '-v', 'ON_ERROR_STOP=1', '-U', 'postgres', '-d', 'billing_test',
  ], { input: sql.join('\n\n'), timeout: 60_000 })

  if (!output.includes('billing_sql_integration_green')) {
    throw new Error(`SQL proof sentinel missing\n${output}`)
  }
  console.log('Billing SQL integration: GREEN')
} finally {
